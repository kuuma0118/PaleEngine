...
//実体定義
GraphicsDirectionCenter* GraphicsDirectionCenter::instance_ = nullptr;

GraphicsDirectionCenter* GraphicsDirectionCenter::GetInstance()
{
	if (instance_ == nullptr)
	{
		instance_ = new GraphicsDirectionCenter();
	}
	return instance_;
}

void GraphicsDirectionCenter::Destroy()
{
	if (instance_)
	{
		delete instance_;
...
void GraphicsDirectionCenter::Initialize()
{
#ifdef _DEBUG
	...
	framePerSsecond_ = std::make_unique<FramePerSsecondGenerate>();
	framePerSsecond_->Initialize();

	//シェーダーコンパイラーの初期化
	ShaderCompiler::Initialize();
}

void GraphicsDirectionCenter::PreDraw()
{
	//バックバッファを取得
...
void GraphicsDirectionCenter::PostDraw()
{
	//バックバッファを取得
	ColorBuffer& currentBackBuffer = display_->GetCurrentBuffer();
	...
	//FPS固定
	framePerSsecond_->Update();

	//次のフレーム用のコマンドリストを準備
	commandContext_->Reset();
}

void GraphicsDirectionCenter::ClearRenderTarget()
{
	//バックバッファを取得
	ColorBuffer currentBackBuffer = display_->GetCurrentBuffer();
...
void GraphicsDirectionCenter::ClearDepthBuffer()
{
	//深度値を書き込めるようにする
...
DescriptorHandle GraphicsDirectionCenter::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE type)
{
